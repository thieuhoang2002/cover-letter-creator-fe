...
function TemplateManager() {
    ...
    const fetchTemplates = async () => {
        try {
            ...
            showSnackbar("Không thể tải danh sách mẫu đơn!", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleOpenDialog = (template = null) => {
        ...
        if (template) {
            ...
            setFormData({
                name: "",
                type: "",
                content: "<p>Nội dung mẫu đơn xin việc bắt đầu từ đây...</p>",
                image: "",
                status: "active"
            });
        }
    ...
    const handleSaveTemplate = async () => {
        if (!formData.name.trim()) {
            showSnackbar("Vui lòng nhập tên mẫu đơn!", "warning");
            return;
        }
        ...
        try {
            ...
            if (isEdit && selectedTemplate) {
                ...
                showSnackbar("Cập nhật mẫu đơn thành công!");
            } else {
                await createTemplate(formData);
                showSnackbar("Tạo mẫu đơn mới thành công!");
            }
            fetchTemplates();
            handleCloseDialog();
        } catch (error) {
            console.error("Lỗi khi lưu mẫu đơn:", error);
            showSnackbar("Có lỗi xảy ra khi lưu mẫu đơn!", "error");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleDeleteTemplate = async () => {
        ...
        try {
            ...
            showSnackbar(`Đã xóa mẫu đơn "${itemToDelete.name}" thành công!`);
            setDeleteModalOpen(false);
            setItemToDelete(null);
            fetchTemplates();
        } catch (error) {
            console.error("Lỗi khi xóa mẫu đơn:", error);
            showSnackbar("Không thể xóa mẫu đơn này!", "error");
        }
    };
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2, py: 1 }}>
                    ...
                    <Box sx={{ overflow: "hidden" }}>
                        ...
                        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 0.5 }}>
                            Phân loại: <strong>{params.row.type || "Chung"}</strong>
                        </Typography>
                    </Box>
                </Box>
            )
            ...
            >
                <Box>
                    <Typography variant="h5" sx={{ fontWeight: 700, letterSpacing: "-0.5px" }}>
                        Quản lý Mẫu Đơn Xin Việc
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                        Tạo mới, chỉnh sửa nội dung mẫu đơn và quản lý hiển thị cho người dùng
                    </Typography>
                </Box>
                ...
                <Grid container spacing={2} alignItems="center">
                    <Grid item xs={12} sm={6} md={5}>
                        <TextField
                            fullWidth
                            size="small"
                            placeholder="Tìm kiếm mẫu đơn theo tên, phân loại..."
                            value={searchTerm}
                ...
                <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1, px: 3 }}>
                    <Box>
                        <Typography variant="h6" sx={{ fontWeight: 700 }}>
                            {isEdit ? "Chỉnh sửa Mẫu Đơn" : "Tạo Mẫu Đơn Mới"}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                            {isEdit ? `ID: #${selectedTemplate?.id} • Chỉnh sửa thông tin và nội dung mẫu` : "Thiết lập thông tin và cấu hình nội dung trình soạn thảo"}
                        </Typography>
                    </Box>
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} md={7}>
                                <TextField
                                    ...
                                    label="Tên Mẫu Đơn *"
                                    value={formData.name}
                                    placeholder="VD: Đơn xin việc chuẩn Chuyên viên IT"
                                    margin="normal"
                                ...
                                <TextField
                                    ...
                                    value={formData.type}
                                    placeholder="VD: Công nghệ thông tin, Kinh doanh, Nhân sự..."
                                    margin="normal"
                                ...
                                <TextField
                                    fullWidth
                                    label="URL Hình ảnh minh họa (Thumbnail)"
                                    value={formData.image}
                                    placeholder="https://example.com/cover-letter-preview.jpg"
                                    margin="normal"
                                    onChange={(e) => setFormData({ ...formData, image: e.target.value })}
                                    helperText="Dán liên kết ảnh đại diện mẫu đơn để người dùng xem trước"
                                />
                            </Grid>
            ...
            >
                ...
                <DialogContent>
                    <Typography variant="body2">
                        Bạn có chắc chắn muốn xóa mẫu đơn <strong>"{itemToDelete?.name}"</strong>?
                    </Typography>
                    <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 1 }}>
                        Thao tác này sẽ gỡ bỏ mẫu đơn khỏi danh sách hệ thống.
                    </Typography>
                </DialogContent>
