...
function ModernCVTemplateManager() {
    ...
    const handleSaveTemplate = async () => {
        try {
            if (isEdit && selectedTemplate) {
                await updateModernTemplate(selectedTemplate.id, newTemplate);
                setSnackbarMessage("Cập nhật mẫu CV hiện đại thành công!");
            } else {
                await createModernTemplate(newTemplate);
                setSnackbarMessage("Thêm mẫu CV hiện đại thành công!");
            }
    ...
    const handleDeleteTemplate = async (id) => {
        try {
            await deleteModernTemplate(id);
            setSnackbarMessage("Xóa mẫu CV hiện đại thành công!");
            setSnackbarOpen(true);
            fetchTemplates();
        } catch (error) {
    ...
    const columns = [
        ...
        { field: "name", headerName: "Tên Mẫu CV", width: 200 },
    ...
    return (
        <Box sx={{ width: "100%", p: 3, background: "linear-gradient(135deg, #f6f9fc 0%, #e3f2fd 100%)" }}>
